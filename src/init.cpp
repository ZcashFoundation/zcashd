...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-regtestallowlegacychainsupplydata", "Skip the chain supply consistency check on blocks where chain supply tracking fields are missing, instead of aborting the node (regtest-only). Needed only for tests that load a regtest cache generated by a zcashd version before v5.4.0");
        ...
        strUsage += HelpMessageOpt("-regtestacceptunvalidatedpow", "Accept block headers without validating the Equihash solution or hash target, for following a Zebra regtest chain whose mined blocks carry null solutions (regtest-only)");
...
bool AppInit2(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (mapArgs.count("-regtestallowlegacychainsupplydata") && chainparams.NetworkIDString() != "regtest") {
        return InitError("-regtestallowlegacychainsupplydata may only be used on regtest.");
    }

    if (mapArgs.count("-regtestchainsupplycheckpoint") && chainparams.NetworkIDString() != "regtest") {
        return InitError("-regtestchainsupplycheckpoint may only be used on regtest.");
    }

    if (mapArgs.count("-regtestenablezip209") && chainparams.NetworkIDString() != "regtest") {
        return InitError("-regtestenablezip209 may only be used on regtest.");
    }

    if (mapArgs.count("-regtestacceptunvalidatedpow") && chainparams.NetworkIDString() != "regtest") {
        return InitError("-regtestacceptunvalidatedpow may only be used on regtest.");
    }
